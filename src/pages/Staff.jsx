...
export default function Staff() {
  ...
  const remove = async () => {
    try {
      const updated = await api.delete(`/admin/shop/staff/${deleteTarget._id}`);
      setStaff(updated);
            ...
            <motion.div key={member._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
              <Card className="p-5 h-full flex flex-col items-center text-center">
                <div className="flex gap-1 self-end -mt-1 -mr-1 mb-1">
                  ...
                  <button onClick={() => setDeleteTarget(member)} className="p-1.5 rounded-lg text-text-faint hover:text-danger hover:bg-danger/10 transition-colors"><TrashIcon className="w-4 h-4" /></button>
                </div>
                <img
